...
/**
 * Английский черновик появился, когда касания перестали ограничиваться
 * русскоязычными рынками. Проверяется не факт перевода, а то, ради чего он
 * делался: чтобы в письмо иностранному адресату не утекла русская строка.
 */

test("английский черновик не содержит кириллицы", () => {
  const result = pitch(report([CRITICAL, MINOR]), "Acme Ltd", "en");
  assert.ok(result.ok);
  const text = result.ok ? result.text : "";

  assert.ok(text.includes("Acme Ltd"), "название компании не подставилось");
  assert.ok(!/[а-яё]/i.test(text), `в английское письмо утекла кириллица: ${text}`);
  assert.ok(/phone/i.test(text), "находка не названа по существу");
});

test("по умолчанию локаль русская — старые вызовы означают то же, что означали", () => {
  const withoutArg = pitch(report([CRITICAL]), "ООО «Ромашка»");
  const explicitRu = pitch(report([CRITICAL]), "ООО «Ромашка»", "ru");
  assert.ok(withoutArg.ok && explicitRu.ok);
  assert.equal(
    withoutArg.ok ? withoutArg.text : "",
    explicitRu.ok ? explicitRu.text : "",
  );
});

test("находка без английского захода не даёт черновика вместо русского текста", () => {
  // Подставить русскую строку в английское письмо — не «лучше, чем ничего»:
  // это сообщение, которое адресат не прочитает, от лица студии, продающей
  // четырёхъязычные сайты. Пустой черновик виден и чинится за пять минут.
  const unknown: Finding = {
    code: "нет_такой_проверки",
    severity: "major",
  ...
  assert.equal(en.ok, false, "английский черновик собрался из русских строк");

  // По-русски запасной вариант есть: заголовок и последствие уже написаны
  // по-русски в самом аудиторе, и они честны.
  const ru = pitch(report([unknown]), "ООО «Ромашка»", "ru");
  assert.ok(ru.ok, "русский черновик должен собираться и без частного захода");
});

test("замер попадает и в английский черновик", () => {
  const slow: Finding = {
    code: "slow",
    severity: "major",
    title: "Сервер отвечает за 4.2 сек",
