 ...
 */
test("менять лид может владелец, руководитель или админ, больше никто", () => {
  const manager = {
    id: "s-1",
    telegram_user_id: 1,
    username: null,
  ...
  const head = { ...manager, id: "s-4", role: "head" as const };
  assert.equal(canEdit({ assigned_staff_id: "s-1" }, head), true, "руководитель — тоже");
