...
test("короткий ответ без вопроса модели не отдаётся", () => {
  ...
  assert.equal(
    readInbound("Здравствуйте! Спасибо за обратную связь, расскажите подробнее, как вы работаете"),
    "talk",
  );
});

test("просьбу прислать отдаём менеджеру, а не модели", () => {
  // Раньше «пришлите разбор на почту» считалось разговором, модель отвечала
  // «подготовлю и пришлю» — и не присылал никто: так заглохла живая
  // переписка с gh.uz. Модель сама ничего не отправит, поэтому такая
  // просьба сразу уходит человеку.
  assert.equal(readInbound("Здравствуйте! Спасибо за обратную связь, пришлите разбор на почту"), "send");
  assert.equal(readInbound("Скиньте КП"), "send");
  assert.equal(readInbound("Отправьте на info@gh.uz в отдел маркетинга"), "send");
  assert.equal(readInbound("Taklifni yuboring iltimos"), "send");
  // Отказ и просьба позвонить — сильнее: их правила стоят раньше.
  assert.equal(readInbound("Не пишите мне больше"), "stop");
  assert.equal(readInbound("Позвоните, а КП потом"), "human");
});
