...
test("«связался сам» доступен до того, как что-то отправлено", () => {
  ...
  // «sending» — тоже: пока письмо ждёт очереди бота, человек может написать
  // сам, и отметка снимает письмо с очереди (иначе клиенту ушло бы два).
  assert.match(store, /SELF_CONTACT_FROM = \["new", "contacting", "manual", "sending"\]/);
