 ...
 */
/**
 * Кадр, после которого считаем, что устройство не справляется.
 *
 * 55 мс — это примерно восемнадцать кадров в секунду. Порог намеренно не
 * «ниже шестидесяти» и даже не «ниже тридцати»: тридцать кадров на
 * декоративном слое — нормальная работа, а не повод резать качество, и
 * устройство, честно держащее тридцать, от понижения ничего не выиграет.
 * Резать нужно там, где картинка уже дёргается.
 */
const SLOW_FRAME_MS = 55;

/** Сколько таких кадров подряд нужно, чтобы понизить качество. */
const SLOW_FRAMES_LIMIT = 40;

/** Кадров от старта, которые не учитываются: страница ещё оживает. */
...
export function CodeRain({
  ...
  useEffect(() => {
    ...
    /**
     * Качество отрисовки: 1 — полное, 0.5 — вдвое меньше точек и капель.
     *
     * Слабое устройство по числу ядер не опознаётся: дешёвые телефоны
     * рапортуют восемь штук и всё равно не тянут. Поэтому уровень не
     * угадывается заранее, а понижается по факту — когда кадры перестали
    ...
    const resize = () => {
      ...
      const dpr = Math.max(1, Math.min(window.devicePixelRatio || 1, narrow ? 1.5 : 2) * quality);
      ...
      const count = Math.max(6, Math.round(Math.min(budget, width / step) * quality));
    ...
    const draw = (now: number) => {
      ...
      // Кадры дольше 32 мс — устройство не укладывается в тридцать в
      // секунду. Считаем идущие подряд и даём счётчику остывать: один
      // тяжёлый кадр бывает у кого угодно — сборка мусора, вкладка
      // вернулась из фона, — и реагировать на него незачем.
      //
      // Первые полторы секунды не в счёт. Пока идёт гидратация и грузятся
      // шрифты, главный поток занят у кого угодно, включая быструю машину,
      // — и без этой паузы дождь уходил в половинное качество на любом
      // устройстве, просто потому что страница только что открылась.
      const delta = last ? now - last : 0;
      last = now;
      if (warmup > 0) warmup -= 1;
      else if (delta > SLOW_FRAME_MS) slow += 1;
      else if (slow > 0) slow -= 1;

      if (slow > SLOW_FRAMES_LIMIT && !degraded) {
        // Один раз и навсегда: половинное разрешение холста и вдвое меньше
        // капель. Второй порог развёл бы качелями — устройство, поймавшее
        // просадку на первом, тут же ловило бы её и на втором.
        degraded = true;
        quality = 0.5;
        resize();
      }
    ...
    const onVisibility = () => {
      ...
      // Кадр после возвращения из фона всегда «долгий»: между ним и
      // предыдущим прошло сколько угодно времени. Обнуляем отсчёт, иначе
      // дождь деградировал бы у каждого, кто отвлёкся на другую вкладку.
      last = 0;
