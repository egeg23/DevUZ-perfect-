 ...
 * Отбор — Соннет.
 *
 * В замере все три модели поймали десять заявок из десяти и не приняли за
 * заявку ни одного из десяти посторонних сообщений, но Хайку на пачке из
 * тридцати вернул двенадцать вердиктов: восемнадцать сообщений остались
 * неразобранными молча, а буфер их обратно не отдаёт. Пачка у нас ровно
 * двадцать, то есть запас нулевой — экономия шести десятых цента не стоит
 * потерянных лидов.
 */
