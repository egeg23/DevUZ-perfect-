...
/**
 * Находки, с которых письмо открывается: про клиентов и деньги.
 *
...
export function outreachPrompt(input: OutreachInput): string {
  ...
  // клиентов и деньги (leadFindings): с них письмо и открывается; за ними —
  // то, из чего сложился балл видимости.
  const lead = leadFindings(input.findings);
  const named = new Set(lead.map((f) => f.code));
  const seoRest = seo.shown.filter((f) => !named.has(f.code));
  for (const f of seoRest) named.add(f.code);
  const findings = [
    ...lead,
    ...seoRest,
    // Одна находка сверх этого, если ничего про клиентов не нашлось: сайт, у
    // которого с Google всё хорошо, иначе получил бы письмо ни о чём.
    ...(lead.length ? [] : input.findings.filter((f) => !named.has(f.code)).slice(0, 1)),
  ]
  ...
  return [
    ...
    "",
    lead.length
      ? `Что нашёл анализ. Первые ${lead.length === 1 ? "одна" : "две"} — про его клиентов и деньги: открывай письмо ими. Остальное — только если останется место (починку не называй — она обсуждается на разборе):`
...
— **Открывай находкой про клиентов и деньги**, если она передана: нет цен, не читается с телефона, телефон не нажимается, сайт не открывается. Техническое — превью ссылки в мессенджере, иконка вкладки, описание страницы для поиска — владелец читает как придирку, и письмо с него не открывают.
— **Не обещай ничего прислать позже** — разбор, смету, файл. Отправить их некому, и обещание, которое не сдержали, хуже молчания. Зови на короткий созвон, где это покажут на его сайте.
