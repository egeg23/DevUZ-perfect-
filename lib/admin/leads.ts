 ...
 *
 * contact_handle — контакт клиента. Панель на этом этапе только читает:
 * механики «лид закреплён за менеджером» ещё нет, а значит любой менеджер
 * выгрузил бы себе все контакты сразу. Ровно то, ради чего эта панель и
 * затевалась вместо переписки в личных телеграмах. Контакт появится
 * вместе с закреплением — в своём разделе и со строкой в журнале.
 *
...
export async function leadCounts(): Promise<{
  ...
  const db = serviceClient();
  if (!db) return { total: 0, fresh: 0, hot: 0, offline: true };

  const head = { count: "exact" as const, head: true };
  const [all, fresh, hot] = await Promise.all([
    db.from("leads").select("id", head),
    db.from("leads").select("id", head).eq("status", "new"),
    db.from("leads").select("id", head).eq("priority", "hot"),
  ]);
