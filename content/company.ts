...
export const stats = [
  {
    // Число берётся из `proof`, а не пишется цифрой: его же называет первое
    // письмо. Стояло «12+ проектов в продакшене» — про то, сколько проектов
    // мы ведём сейчас, а письмо говорило про 550 завершённых. Разные вещи, но
    // адресат, перешедший по ссылке, разницы не разбирает: он видит два
    // числа, расходящихся в сорок раз, и дальше не верит ни баллу, ни расчёту
    // потерь, ни всему остальному, что мы честно измерили.
