...
async function handleButton(query: NonNullable<Update["callback_query"]>) {
  ...
  // Кнопки под карточкой лида — в чате отдела продаж и в личке сотрудника.
  //
  // Раньше здесь было «только чат продаж»: карточки висели там одни. С тех
  // пор карточка приходит каждому в личку, а предложение очереди — «лид
  // ваш на 30 минут» — только в личку, и «Взять в работу» под ним отвечало
  // «Недоступно» самому адресату. Личка — это чат, где id чата совпадает с
  // id нажавшего: туда бот пишет сам, и посторонний в неё не попадёт. Любая
  // другая группа — по-прежнему «Недоступно».
  if (chatId !== undefined && !isSalesChat(chatId) && chatId !== query.from?.id) {
    await answerCallback(query.id, "Недоступно");
    return;
  }
